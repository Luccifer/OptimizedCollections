...
extension BTree.Node {
    func split() -> BTree<Element>.Splinter {
        ...
        let node = BTree<Element>.Node(order: self.order)
...
extension BTree.Node {
    func _splittingInsert(_ element: Element, with child: BTree<Element>.Node? = nil, at slot: Int) -> BTree<Element>.Splinter? {
...
extension BTree.Node {
    private func _inserting(_ element: Element, with neighbors: (BTree<Element>.Node, BTree<Element>.Node)? = nil, at slot: Int) -> (trunk: BTree<Element>.Node, splinter: BTree<Element>.Splinter?) {
        if elementCount < maxElements {
            let trunk = BTree<Element>.Node(order: order)
        ...
        let separator: Element
        let left = BTree<Element>.Node(order: order)
        let right = BTree<Element>.Node(order: order)
        ...
        return (left, BTree<Element>.Splinter(separator: separator, node: right))
    }
    
    func inserting(_ element: Element) -> (old: Element?, trunk: BTree<Element>.Node, splinter: BTree<Element>.Splinter?) {
        let slot = self.slot(of: element)
        if slot.match {
            // The element is already in the tree.
...
extension BTree.Node {
    convenience init(trunk: BTree<Element>.Node, splinter: BTree<Element>.Splinter) {
